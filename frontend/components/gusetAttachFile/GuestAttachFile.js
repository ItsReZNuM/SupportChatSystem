...
export default function GuestAttachFile({ conversationId, setConversationId, guestId, setGuestId }) {
        ...
        try {
            ...
            if (!sessionRaw) {
                ...
                localStorage.setItem(
                    "chat_session_key",
                    JSON.stringify({ guest_id, conversation_id: id }),
                );
                setGuestId(guest_id);
            ...
            const { guest_id, conversation_id } = JSON.parse(sessionRaw);

            const formData = new FormData();
            formData.append("file", selectedFile);
            const uploadRes = await api.post(
                `/chat/conversations/upload/${conversation_id}`,
                formData,
            );
    ...
    return (
        <div className="attach-file">
            ...
            >
                <div className="main bg-gray-800 w-100 h-auto rounded-lg p-3 flex flex-col items-center gap-5">
